...
class QualityBuilder():
    ...
    def __init__(self, hydraulics, superjunction_params, superlink_params,
        ...
        if link_params is not None:
            ...
            self._K_ik = superlink_params['K'].values[self._ki].astype(np.float64)
            self._c_ik = superlink_params['c_0'].values[self._ki].astype(np.float64)
        self._dx_uk = self.hydraulics._dx_uk  #★★★
        self._dx_dk = self.hydraulics._dx_dk  #★★★       
        ...
        self._is_start = self.hydraulics._is_start   # Index of starting link of superlinks
        self._is_end = self.hydraulics._is_end       # Index of ending link of superlinks
        ...
        self.NK = self.hydraulics.NK                 # Number of superlinks
        self.M = self.hydraulics.M                   # Number of superjunctions
        self.nk = self.hydraulics.nk                 # Number of links
        ...
        self.Observation_point = superlink_params['KF_obs_point'].values[0].astype(np.int32)
    ...
    def import_hydraulic_states(self, _dt):
        ...
        self._z_inv_j = self.hydraulics.z_inv_j  #★★★
        self._V_j_next = self._A_sj*(self._H_j_next - self._z_inv_j)
        self._V_j_prev = self._A_sj*(self._H_j_prev - self._z_inv_j)
        self._B_uk = self.hydraulics._B_uk
        self._B_dk = self.hydraulics._B_dk
        self._B_ik = self.hydraulics._B_ik
        self._Ik = self.hydraulics._Ik
        self._Ip1k = self.hydraulics._Ip1k
                
        # Calculate the cross-sectional area of links
        Ix_ik = np.arange(len(self._dx_ik_next))
        if self.hydraulics._geom_codes[0] ==1:
            d = self.hydraulics._g1_ik
            y = 0.5*(self._h_Ik_next[self._Ik] + self._h_Ik_next[self._Ip1k])
            y[y < 0] = 0
            y[y > d] = d[y > d]
            r = d / 2
            phi = y / r
            ...
            self._A_ik_next = r**2 * (theta - np.cos(theta) * np.sin(theta))
            y = 0.5*(self._h_Ik_prev[self._Ik] + self._h_Ik_prev[self._Ip1k])
            y[y < 0] = 0
            y[y > d] = d[y > d]
            r = d / 2
            phi = y / r
            ...
            phi[phi > 2] = 2
            theta = np.arccos(1 - phi)
            self._A_ik_prev = r**2 * (theta - np.cos(theta) * np.sin(theta))
        elif self.hydraulics._geom_codes[0] ==3:
            self._A_ik_next = self._B_ik[Ix_ik]*0.5*(self._h_Ik_next[self._Ik] + self._h_Ik_next[self._Ip1k])
            self._A_ik_prev = self._B_ik[Ix_ik]*0.5*(self._h_Ik_prev[self._Ik] + self._h_Ik_prev[self._Ip1k])
        else:
            raise ValueError('Geometries of conduit should be [circular] or [rect_open] in this version of WQ solver.')
        self._Q_ik_up_next = (self._Q_ik_next[Ix_ik] + (0.5*self._B_ik[Ix_ik]*self._dx_ik_next[Ix_ik])
                        *(self._h_Ik_next[self._Ik] - self._h_Ik_prev[self._Ik])/_dt)  #★★★
        self._Q_ik_dn_next = (self._Q_ik_next[Ix_ik] - (0.5*self._B_ik[Ix_ik]*self._dx_ik_next[Ix_ik])
                        *(self._h_Ik_next[self._Ip1k] - self._h_Ik_prev[self._Ip1k])/_dt)  #★★★
        _kI = self._kI
        ...
        self._Q_uk_dn_next = np.zeros(len(self.nk))
        self._Q_dk_up_next = np.zeros(len(self.nk))
        self._Q_dk_dn_next = np.zeros(len(self.nk))
        self._A_uk_next = np.zeros(len(self.nk))
        ...
        for I in range(N):
            if _is_start[I]:
                i = _forward_I_i[I]
                k = _kI[I]
                self._Q_uk_up_next[k] = (self._Q_uk_next[k] + (0.5*self._B_uk[k]*self._dx_uk[k])
                                        *(self._H_j_next[_J_uk[k]] - self._H_j_prev[_J_uk[k]])/_dt)
                self._Q_uk_dn_next[k] = (self._Q_uk_next[k] - (0.5*self._B_uk[k]*self._dx_uk[k])
                                        *(self._h_Ik_next[i] - self._h_Ik_prev[i])/_dt)
                # Calculate the cross-sectional area of upstream boundary links
                if self.hydraulics._geom_codes[0] ==1:
                    d = self.hydraulics._g1_ik[k]
                    y = 0.5*(self._H_j_next[_J_uk[k]] - self._z_inv_j[_J_uk[k]] + self._h_Ik_next[i])
                    if y < 0:
                        y = 0.0
                    elif y > d:
                        y = d
                    ...
                    if phi < 0:
                        ...
                        phi = 2
                    theta = np.arccos(1 - phi)
                    self._A_uk_next[k] = r**2 * (theta - np.cos(theta) * np.sin(theta))
                    y = 0.5*(self._H_j_prev[_J_uk[k]] - self._z_inv_j[_J_uk[k]] + self._h_Ik_prev[i])
                    if y < 0:
                        y = 0.0
                    elif y > d:
                        y = d
                    ...
                    if phi < 0:
                        ...
                        phi = 2
                    theta = np.arccos(1 - phi)
                    self._A_uk_prev[k] = r**2 * (theta - np.cos(theta) * np.sin(theta))
                elif self.hydraulics._geom_codes[0] ==3:
                    self._A_uk_next[k] = (self._B_uk[k]*0.5*(self._H_j_next[_J_uk[k]] - self._z_inv_j[_J_uk[k]] + self._h_Ik_next[i]))
                    self._A_uk_prev[k] = (self._B_uk[k]*0.5*(self._H_j_prev[_J_uk[k]] - self._z_inv_j[_J_uk[k]] + self._h_Ik_prev[i]))
                else:
                    raise ValueError('Geometries of conduit should be [circular] or [rect_open] in this version of WQ solver.')
                ...
                im1 = _backward_I_i[I]
                k = _kI[I]
                self._Q_dk_up_next[k] = (self._Q_dk_next[k] + (0.5*self._B_dk[k]*self._dx_dk[k])
                                      *(self._h_Ik_next[im1] - self._h_Ik_prev[im1])/_dt)
                self._Q_dk_dn_next[k] = (self._Q_dk_next[k] - (0.5*self._B_dk[k]*self._dx_dk[k])
                                        *(self._H_j_next[_J_dk[k]] - self._H_j_prev[_J_dk[k]])/_dt)
                
                # Calculate the cross-sectional area of downstream boundary links
                if self.hydraulics._geom_codes[0] ==1:
                    d = self.hydraulics._g1_ik[k]
                    y = 0.5*(self._H_j_next[_J_dk[k]] - self._z_inv_j[_J_dk[k]] + self._h_Ik_next[im1])
                    if y < 0:
                        y = 0.0
                    elif y > d:
                        y = d
                    ...
                    if phi < 0:
                        ...
                        phi = 2
                    theta = np.arccos(1 - phi)
                    self._A_dk_next[k] = r**2 * (theta - np.cos(theta) * np.sin(theta))
                    y = 0.5*(self._H_j_prev[_J_dk[k]] - self._z_inv_j[_J_dk[k]] + self._h_Ik_prev[im1])
                    if y < 0:
                        y = 0.0
                    elif y > d:
                        y = d
                    ...
                    if phi < 0:
                        ...
                        phi = 2
                    theta = np.arccos(1 - phi)
                    self._A_dk_prev[k] = r**2 * (theta - np.cos(theta) * np.sin(theta))
                elif self.hydraulics._geom_codes[0] ==3:
                    self._A_dk_next[k] = (self._B_dk[k]*0.5*(self._H_j_next[_J_dk[k]] - self._z_inv_j[_J_dk[k]] + self._h_Ik_next[im1]))
                    self._A_dk_prev[k] = (self._B_dk[k]*0.5*(self._H_j_prev[_J_dk[k]] - self._z_inv_j[_J_dk[k]] + self._h_Ik_prev[im1]))
                else:
                    raise ValueError('Geometries of conduit should be [circular] or [rect_open] in this version of WQ solver.')
            else:
                pass
    ...
    def link_coeffs(self, _dt=None, first_iter=True):
        ...
        """
        # Import instance variables
        _Q_ik_up_next = self._Q_ik_up_next
        _Q_ik_dn_next = self._Q_ik_dn_next
        ...
        _Q_uk_dn_next = self._Q_uk_dn_next
        _Q_dk_up_next = self._Q_dk_up_next
        _Q_dk_dn_next = self._Q_dk_dn_next
        ...
        if _dt is None:
            _dt = self._dt
        _omega_ik_up = (_Q_ik_up_next > 0).astype(np.float64)
        _omega_ik_dn = (_Q_ik_dn_next > 0).astype(np.float64)
        _omega_uk_up = (_Q_uk_up_next > 0.).astype(np.float64)
        _omega_uk_dn = (_Q_uk_dn_next > 0.).astype(np.float64)
        _omega_dk_up = (_Q_dk_up_next > 0.).astype(np.float64)
        _omega_dk_dn = (_Q_dk_dn_next > 0.).astype(np.float64)
        _alpha_ik = alpha_ik(_Q_ik_up_next, _omega_ik_up, _dx_ik_next, _D_ik, _A_ik_next)
        _beta_ik = beta_ik(_dt, _D_ik, _A_ik_next, _A_ik_prev, _dx_ik_next, _K_ik,
                           _Q_ik_up_next, _Q_ik_dn_next, _omega_ik_up, _omega_ik_dn)
        _chi_ik = chi_ik(_Q_ik_dn_next, _omega_ik_dn, _dx_ik_next, _D_ik, _A_ik_next)
        _gamma_ik = gamma_ik(_dt, _c_ik, _A_ik_next, _dx_ik_next)
        # Compute link coefficients for upstream boundary
        _alpha_uk = alpha_ik(_Q_uk_up_next, _omega_uk_up, _dx_uk, _D_uk, _A_uk_next)
        _beta_uk = beta_ik(_dt, _D_uk, _A_uk_next, _A_uk_prev, _dx_uk, 0, _Q_uk_up_next, _Q_uk_dn_next,
                           _omega_uk_up, _omega_uk_dn)
        _chi_uk = chi_ik(_Q_uk_dn_next, _omega_uk_dn, _dx_uk, _D_uk, _A_uk_next)
        _gamma_uk = gamma_ik(_dt, _c_uk, _A_uk_next, _dx_uk)
        # Compute link coefficients for downstream boundary
        _alpha_dk = alpha_ik(_Q_dk_up_next, _omega_dk_up, _dx_dk, _D_dk, _A_dk_next)
        _beta_dk = beta_ik(_dt, _D_dk, _A_dk_next, _A_dk_prev, _dx_dk, 0, _Q_dk_up_next, _Q_dk_dn_next,
                           _omega_dk_up, _omega_dk_dn)
        _chi_dk = chi_ik(_Q_dk_dn_next, _omega_dk_dn, _dx_dk, _D_dk, _A_dk_next)
    ...
    def node_coeffs(self, _Q_0Ik=None, _c_0Ik=None, _dt=None, first_iter=True):
        ...
        _omega_dk_up = (_Q_dk_up_next > 0.).astype(np.float64)
        _omega_dk_dn = (_Q_dk_dn_next > 0.).astype(np.float64)
        numba_node_coeffs(_kappa_Ik, _lambda_Ik, _mu_Ik, _eta_Ik, _Q_ik_up_next, _Q_ik_dn_next,
                          _h_Ik_next, _h_Ik_prev, _c_Ik_prev, _c_ik_prev,
                          _c_0Ik, _Q_0Ik, _A_SIk,
                          _K_Ik, _K_ik, _A_ik_next, _dx_ik_next,
                          _forward_I_i, _backward_I_i, _is_start, _is_end, _kI,
                          _dt, _omega_ik_up, _omega_ik_dn, _omega_uk_up, _omega_uk_dn, _omega_dk_up, _omega_dk_dn,
                          _D_ik, _dx_uk, _dx_dk, _D_uk, _D_dk,
                          _A_uk_next, _A_dk_next, _Q_uk_up_next,_Q_uk_dn_next,_Q_dk_up_next,_Q_dk_dn_next)
    ...
    def sparse_matrix_equations(self, c_bc=None, _Q_0j=None, _c_0j=None, u=None, _dt=None,
        ...
        numba_create_A_matrix(A, _F_jj, bc, _J_uk, _J_dk, _rho_uk, _rho_dk, _tau_uk, _tau_dk,
                              _Q_uk_up_next, _Q_dk_dn_next, _omega_uk_up, _omega_dk_dn, _V_j_next, _V_j_prev,
                              _H_j_next, _dt, _K_j, _D_uk, _D_dk, _A_uk_next, _A_dk_next,
                              _dx_uk, _dx_dk, M, NK)
        # Create D vector
        numba_add_at(D, _J_uk, (-_Q_uk_up_next * (1 - _omega_uk_up)
                                + 2 * _D_uk * _A_uk_next / _dx_uk) * _zeta_uk)
        numba_add_at(D, _J_dk, (_Q_dk_dn_next * _omega_dk_dn
    ...
    def step(self, dt=None, c_bc=None, c_0j=None, Q_0j=None, c_0Ik=None,
        ...
        None
        """
        self.import_hydraulic_states(_dt=dt)
        if dt is None:
            dt = self._dt
        if Q_0j is None:
...
@njit
def alpha_ik(Q_ik_up, omega_ik_up, dx_ik, D_ik, A_ik):
    # Use upwind scheme
    t_0 = - Q_ik_up * omega_ik_up
    t_1 = - 2 * D_ik * A_ik / dx_ik
    return t_0 + t_1

@njit
def beta_ik(dt, D_ik, A_ik, A_ik_prev, dx_ik, K_ik, Q_ik_up, Q_ik_dn, omega_ik_up, omega_ik_dn):
    t_0 = Q_ik_dn * omega_ik_dn
    t_1 = - Q_ik_up * (1 - omega_ik_up)
...
@njit
def chi_ik(Q_ik_dn, omega_ik_dn, dx_ik, D_ik, A_ik):
    # Use upwind scheme
    t_0 = Q_ik_dn * (1 - omega_ik_dn)
    t_1 = - 2 * D_ik * A_ik / dx_ik
    return t_0 + t_1
...
@njit
def kappa_Ik(Q_im1k_dn_next, omega_dn_im1k, D_Ik, A_Ik, dx_im1k):
    t_0 = - Q_im1k_dn_next * omega_dn_im1k
    t_1 = - 2 * D_Ik * A_Ik / dx_im1k
    return t_0 + t_1

@njit
def lambda_Ik(A_SIk, h_Ik_next, h_Ik_prev, Q_ik_up_next, Q_im1k_dn_next, omega_ik_up, omega_im1k_dn, dt,
              K_Ik, D_ik, D_im1k, A_ik, A_im1k, dx_ik, dx_im1k):
    t_0 = A_SIk * h_Ik_next * K_Ik
    t_1 = omega_ik_up * Q_ik_up_next
    t_2 = - (1 - omega_im1k_dn) * Q_im1k_dn_next
...
@njit
def mu_Ik(Q_ik_up_next, omega_ik_up, D_Ik, A_Ik, dx_ik):
    t_0 = Q_ik_up_next * (1 - omega_ik_up)
    t_1 = - 2 * D_Ik * A_Ik / dx_ik
    return t_0 + t_1
...
@njit
def numba_node_coeffs(_kappa_Ik, _lambda_Ik, _mu_Ik, _eta_Ik, _Q_ik_up_next, _Q_ik_dn_next,
                      _h_Ik_next, _h_Ik_prev, _c_Ik_prev, _c_ik_prev,
                      _c_0Ik, _Q_0Ik, _A_SIk, _K_Ik,
                      _K_ik, _A_ik_next, _dx_ik_next, _forward_I_i,
                      _backward_I_i, _is_start, _is_end, _kI, _dt, _omega_ik_up, _omega_ik_dn,
                      _omega_uk_up, _omega_uk_dn, _omega_dk_up, _omega_dk_dn, _D_ik, _dx_uk, _dx_dk,
                      _D_uk, _D_dk, _A_uk_next, _A_dk_next, _Q_uk_up_next,_Q_uk_dn_next,_Q_dk_up_next,_Q_dk_dn_next):
    N = _kI.size
    for I in range(N):
        if _is_start[I]:
            i = _forward_I_i[I]
            k = _kI[I]
            _kappa_Ik[I] = kappa_Ik(_Q_uk_dn_next[k], _omega_uk_dn[k], _D_uk[k], _A_uk_next[k], _dx_uk[k])
            _lambda_Ik[I] = lambda_Ik(_A_SIk[I], _h_Ik_next[I], _h_Ik_prev[I], _Q_ik_up_next[i],
                                      _Q_uk_dn_next[k], _omega_ik_up[i], _omega_uk_dn[k], _dt, _K_Ik[I],
                                      _D_ik[i], _D_uk[k], _A_ik_next[i], _A_uk_next[k],
                                      _dx_ik_next[i], _dx_uk[k])
            _mu_Ik[I] = mu_Ik(_Q_ik_up_next[i], _omega_ik_up[i], _D_ik[i], _A_ik_next[i], _dx_ik_next[i])
            _eta_Ik[I] = eta_Ik(_c_0Ik[I], _Q_0Ik[I], _A_SIk[I], _h_Ik_next[I], _c_Ik_prev[I], _dt)
        elif _is_end[I]:
            im1 = _backward_I_i[I]
            k = _kI[I]
            # TODO: Possible out-of-bounds error
            _kappa_Ik[I] = kappa_Ik(_Q_ik_dn_next[im1], _omega_ik_dn[im1], _D_ik[im1], _A_ik_next[im1],
                                    _dx_ik_next[im1])
            _lambda_Ik[I] = lambda_Ik(_A_SIk[I], _h_Ik_next[I], _h_Ik_prev[I], _Q_dk_up_next[k],
                                      _Q_ik_dn_next[im1], _omega_dk_up[k], _omega_ik_dn[im1], _dt, _K_Ik[I],
                                      _D_dk[k], _D_ik[im1], _A_dk_next[k], _A_ik_next[im1],
                                      _dx_dk[k], _dx_ik_next[im1])
            _mu_Ik[I] = mu_Ik(_Q_dk_up_next[k], _omega_dk_up[k], _D_dk[k], _A_dk_next[k], _dx_dk[k])
            ...
            _kappa_Ik[I] = kappa_Ik(_Q_ik_dn_next[im1], _omega_ik_dn[im1], _D_ik[im1], _A_ik_next[im1],
                                    _dx_ik_next[im1])
            _lambda_Ik[I] = lambda_Ik(_A_SIk[I], _h_Ik_next[I], _h_Ik_prev[I], _Q_ik_up_next[i],
                                      _Q_ik_dn_next[im1], _omega_ik_up[i], _omega_ik_dn[im1], _dt,
...
def numba_create_A_matrix(A, _F_jj, bc, _J_uk, _J_dk, _rho_uk, _rho_dk, _tau_uk, _tau_dk,
                          _Q_uk_up, _Q_dk_dn, _omega_uk_up, _omega_dk_dn, _V_j_next, _V_j_prev, _H_j_next, _dt,
                          _K_j, _D_uk, _D_dk, _A_uk_next, _A_dk_next, _dx_uk, _dx_dk, M, NK):
    # TODO: Grouping of Q multiplication changed
    kuj = (- _Q_uk_up * (_omega_uk_up + (1 - _omega_uk_up) * _rho_uk)
           + 2 * _D_uk * _A_uk_next * (_rho_uk - 1) / _dx_uk)
    kdj = (_Q_dk_dn * (_tau_dk * _omega_dk_dn + (1 - _omega_dk_dn))
    ...
    for k in range(NK):
        ...
        _bc_d = bc[_J_d]
        if not _bc_u:
            A[_J_u, _J_d] -= _tau_uk[k] * (-_Q_uk_up[k] * (1 - _omega_uk_up[k])
                              + 2 * _D_uk[k] * _A_uk_next[k] / _dx_uk[k])
        if not _bc_d:
            A[_J_d, _J_u] -= _rho_dk[k] * (_Q_dk_dn[k] * _omega_dk_dn[k]
